(function () {
	...
	function updateResultUI(container, quantity, result, thickness) {
		...
		container.querySelector('[data-gsc-result-volume]').textContent = formatNumber(result.volumeNeeded, 2);
		container.querySelector('[data-gsc-result-bags]').textContent = String(result.bagsNeeded);

		const note = container.querySelector('[data-gsc-note]');
		if (note) {
			note.textContent = gscData.i18n.calculationNote.replace('%s', formatNumber(thickness, 1));
		}

		const addToCartButton = container.querySelector('[data-gsc-add-to-cart]');
		if (addToCartButton && quantity) {
			const type = quantity.bag_type ? quantity.bag_type : gscData.i18n.bagFallback;
	...
	function buildAddToCartUrl(productId, variationId, formatSlug, quantitySlug) {
		...
		if (formatSlug) {
			url.searchParams.set(`attribute_${gscData.attributeFormaat}`, String(formatSlug));
		}
		url.searchParams.set(`attribute_${gscData.attributeQty}`, String(quantitySlug));
		return url.toString();
	}
	...
	function initCalculator(container) {
		...
		function getSelectedQuantity() {
			...
			const quantities = quantitiesByFormat[selectedFormat] || [];
			const selected = quantities.find((item) => item.slug === selectedQuantitySlug) || null;
			if (!selected) {
				return null;
			}

			if (selected.volume_per_bag && selected.weight_per_bag && selected.bag_type) {
				return selected;
			}
		...
		function recalculate() {
			...
			const result = calculateResult(sqmInput.value, thicknessInput.value, quantity);
			updateResultUI(container, quantity, result, thicknessInput.value || config.layerThickness || 5);
		}
		...
		addToCartButton.addEventListener('click', async () => {
			...
			if (!selectedQuantity || !quantitySelect.value || (config.hasFormats && !formatSelect.value)) {
				addMessage(container, gscData.i18n.needSelections);
				return;
			}

			const result = calculateResult(sqmInput.value, thicknessInput.value, selectedQuantity);
			...
			const addUrl = buildAddToCartUrl(config.productId, response.data.variation_id, formatSelect.value, quantitySelect.value);
	...
	function initWizard(container) {
		const configRaw = container.getAttribute('data-config');
		const config = configRaw ? JSON.parse(configRaw) : { categories: [], productsByCategory: {} };
		const stepElements = {
		...
		const categoryCardsContainer = container.querySelector('[data-gsc-category-cards]');
		const productCardsContainer = container.querySelector('[data-gsc-product-cards]');
		const formatCardsContainer = container.querySelector('[data-gsc-format-cards]');
		const quantityOptionsContainer = container.querySelector('[data-gsc-quantity-options]');
		const sqmInput = container.querySelector('[data-gsc-sqm]');
		const thicknessInput = container.querySelector('[data-gsc-thickness]');
		const addToCartButton = container.querySelector('[data-gsc-add-to-cart]');
		const state = {
		...
		function setStep(stepNumber) {
			Object.keys(stepElements).forEach((key) => {
				...
				element.classList.toggle('is-active', Number(key) === Number(stepNumber));
			});
		...
		function getResult() {
			...
			const bestQuantity = findBestQuantity(volumeNeeded, state.availableQuantities || []);
			if (bestQuantity) {
				const parsed = parseQuantityLabel(bestQuantity.label || '') || bestQuantity;
			...
			return calculateResult(sqmInput.value, thicknessInput.value, state.quantityData);
		}

		function recalculate() {
			const result = getResult();
			updateResultUI(container, state.quantityData, result, thicknessInput.value || 5);
		}
		...
		function renderFormats(formats) {
			...
			formats.forEach((format) => {
				...
				button.addEventListener('click', async () => {
					formatCardsContainer.querySelectorAll('.gsc-card').forEach((card) => card.classList.remove('is-selected'));
		...
		function renderCategories() {
			...
			categories.forEach((category) => {
				...
				button.addEventListener('click', () => {
					categoryCardsContainer.querySelectorAll('.gsc-card').forEach((card) => card.classList.remove('is-selected'));
					button.classList.add('is-selected');
		...
		function renderProductsForCategory() {
			...
			const products = (config.productsByCategory && config.productsByCategory[String(state.categoryId)]) || [];
			if (!products.length) {
			...
			products.forEach((product) => {
				...
				button.addEventListener('click', async () => {
					productCardsContainer.querySelectorAll('.gsc-card').forEach((card) => card.classList.remove('is-selected'));
					button.classList.add('is-selected');
		...
		addToCartButton.addEventListener('click', async () => {
			...
			const addUrl = buildAddToCartUrl(state.productId, response.data.variation_id, state.format || '', state.quantity);
			window.location.href = `${addUrl}&quantity=${result.bagsNeeded}`;
		});
